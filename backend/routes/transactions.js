...
router.get('/stats', auth, async (req, res) => {
  try {
    ...
    const transactions = await Transaction.find({ user: req.user }); // Changed from req.user.id
...
router.get('/chart-data', auth, async (req, res) => {
  try {
    console.log('Chart data user:', req.user); // Debug log
    ...
    console.log('Chart transactions found:', transactions.length); // Debug log
    ...
    console.log('Chart data response:', chartData); // Debug log
    res.json(chartData);
  } catch (err) {
...
async function generatePDFReport(doc, transactions, stats) {
  ...
  checkNewPage(200);
  doc.fontSize(14).fillColor('#2D3748').text('MONTHLY TRENDS (Last 6 Months)', 50, yPosition);
  yPosition += 25;
  ...
  for (let i = 1; i <= pageCount; i++) {
    doc.switchToPage(i);
    doc.fontSize(8).fillColor('#999').text(`Page ${i} of ${pageCount} | Generated by Personal Finance Tracker`, 50, 750, { align: 'center' });
  }
}
